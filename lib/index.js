...
function createAccessLogger(options) {
    ...
    // with the default name `access`
        ...
        logger = require('rapidus').getLogger(options.name || options || 'access');
    }
    ...
    // Export middleware as property on logger and return it
    return (logger.middleware = loggerMiddleware);
}
